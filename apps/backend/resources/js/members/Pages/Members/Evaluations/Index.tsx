...
export default function EvaluationsIndex({ pageTitle, subtitle, items }: EvaluationsIndexProps) {
    const navigation: NavItem[] = [
        { label: 'Panel', href: '/miembros-v2' },
        { label: 'Evaluaciones', href: '/miembros-v2/evaluaciones', active: true },
                ...
                <div className="grid gap-4">
                    {items.map((item) => (
                        ...
                        >
                            ...
                            <div className="flex items-center gap-3">
                                <Link
                                    href={`/miembros-v2/evaluaciones/${item.id}`}
                                    className="rounded-full bg-blue-600 px-5 py-3 text-sm font-bold text-white no-underline shadow-lg shadow-blue-500/25 transition hover:bg-blue-700"
                                >
                                    Abrir evaluación
                                </Link>
                            </div>
                    ...
                    {items.length === 0 ? (
                        <div className="rounded-[24px] border border-dashed border-slate-300 bg-slate-50 px-5 py-6 text-sm leading-6 text-slate-600">
                            No tienes evaluaciones visibles en este momento. Cuando el docente active una para tu curso, aparecerá aquí.
                        </div>
                    ) : null}
                </div>
            </section>
