...
export default function WorkshopsIndex({ pageTitle, subtitle, items }: WorkshopsIndexProps) {
    const navigation: NavItem[] = [
        { label: 'Panel', href: '/miembros-v2' },
        { label: 'Evaluaciones', href: '/miembros-v2/evaluaciones' },
        { label: 'Talleres', href: '/miembros-v2/talleres', active: true },
                ...
                <div className="grid gap-4">
                    {items.map((item) => (
                        ...
                        >
                            ...
                            <div className="flex items-center gap-3">
                                <Link
                                    href={`/miembros-v2/talleres/${item.id}`}
                                    className="rounded-full bg-emerald-600 px-5 py-3 text-sm font-bold text-white no-underline shadow-lg shadow-emerald-500/25 transition hover:bg-emerald-700"
                                >
                                    Abrir taller
                                </Link>
                            </div>
                    ...
                    {items.length === 0 ? (
                        <div className="rounded-[24px] border border-dashed border-slate-300 bg-slate-50 px-5 py-6 text-sm leading-6 text-slate-600">
                            No hay talleres visibles en este momento.
                        </div>
                    ) : null}
                </div>
            </section>
