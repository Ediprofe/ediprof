...
class AssessmentQuestionForm
{
    ...
    public static function createSchema(Schema $schema): Schema
    {
        return $schema->components([
            static::contextSection(),
            static::contentSection(),
            static::classificationSection(required: true),
        ]);
    }

    public static function contextSection(): Section
    {
        return Section::make('Contexto base del bloque')
            ->description('Aquí defines la base general del caso, lectura, tabla o situación. Por detrás, el sistema lo guarda dentro del bloque editorial correspondiente.')
            ->components([
                TextInput::make('block_title')
                    ->label('Nombre del bloque')
                    ->helperText('Opcional. Si lo dejas vacío, el sistema tomará el contexto o el enunciado para nombrarlo.')
                    ->maxLength(255),
                TextInput::make('context_title')
                    ->label('Título del contexto')
                    ->helperText('Opcional. Útil cuando el contexto necesita un nombre corto dentro del bloque.')
                    ->maxLength(255),
    ...
    {
        return Section::make('Identidad editorial')
            ->components([
    ...
    {
        return Section::make('Contenido de la pregunta')
            ->description('Aquí trabajas lo evaluable: enunciado, opciones, correcta, retroalimentación y conceptos relacionados.')
            ->components([
                ...
                Placeholder::make('stem_preview')
                    ...
                    ->content(fn (?AssessmentQuestion $record): string => $record instanceof AssessmentQuestion
                        ...
                        : '')
                    ->hidden(fn (?AssessmentQuestion $record): bool => $record === null)
                    ->columnSpanFull(),
    ...
    public static function classificationSection(bool $required = false): Section
    {
        return Section::make('Clasificación editorial')
            ->components([
                ...
                Select::make('origin_collection_id')
                    ->label('Origen')
                    ->helperText('El origen guarda la trazabilidad editorial de la pregunta dentro del banco.')
    ...
    public static function optionsEditor(): Repeater
    {
        return Repeater::make('options_editor')
            ->label('Opciones')
